...
private:
    ...
    void SortBucket()
    {
        ...
        if (!b.started) {  // first start
            b.future = std::async(&SortManager::AsyncSortBucket, this, bucket_i);
            b.started = 1;
        }
        b.future.wait();
        ...
        b.started = 0;
        for (int k = 1; k < 3; k++) {
            if (bucket_i + k >= buckets_.size()) {  // run async
                break;
            }
            bucket_t& next = buckets_[bucket_i + k];
            if (next.started) {
                continue;
            }
            next.future = std::async(&SortManager::AsyncSortBucket, this, bucket_i + k);
            next.started = 1;
        }
